...
/**
 * android framwork层提供GL10来获得相应的参数，而GL10要在使用自定义的View时才可以获得。所以获取GPU信息需要以下几步：
 * （1）新建一个GLSurfaceView;
 * （2）为其添加一个Renderer;
 * （3）重写Renderer中的onSurfaceCreated，在此方法中获得GPU的信息，将获得的信息存起来;
 * @return
 */
public class DeviceInfo extends Activity implements GLSurfaceView.Renderer { // renderer渲染器
	private TextView textView;
	private GLSurfaceView glSurfaceView;
	private StringBuilder sb;
	public String str;
	private ConfigurationInfo configurationInfo; // 配置信息类
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		sb.append("系统版本:").append(android.os.Build.VERSION.RELEASE)
				.append("\n");
		...
		sb.append("GPU型号:").append(gl.glGetString(GL10.GL_RENDERER))
				.append("\n");
		sb.append("GPU制造商:").append(gl.glGetString(GL10.GL_VENDOR))
				.append("\n");
		sb.append("OpenGL版本:").append(configurationInfo.getGlEsVersion())
				.append("\n");
		sb.append("OpenGL ES版本:").append(gl.glGetString(GL10.GL_VERSION))
				.append("\n");
	...
	public static float[] getSpace(Context context) {
		...
		long total_space;
		long other_space;
		float Space[] = new float[6]; // 保存存储信息的数组

		// 获取外部存储可用空间
		sd_freeSpace_path = Environment.getExternalStorageDirectory()
				.getFreeSpace();

		// 将byte转换成MB,1MB=1024KB=1048576Byte
		...
		Space[5] = (float) (total_space / 1048576);
		// 获得其他文件占用的空间
		other_space = total_space
				- (sd_freeSpace_path + rom_freeSpace_path + appSpaceUser + appSpaceSys);
