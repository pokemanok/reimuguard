...
import com.pokeman.reimuguard.info.ProcessInfo;
import com.pokeman.reimuguard.utils.ConstantValue;
import com.pokeman.reimuguard.utils.ProcessInfoProvider;
 ...
 *
 */
public class ProcessManagerPager extends BasePager implements OnClickListener {
	...
	public void initData() {
		...
		//要给帧布局填充对象
		mview = View.inflate(mActivity, R.layout.pager_process, null);
	...
	class MyAdapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if(type == 0){
				...
				if(convertView == null){
					convertView = View.inflate(mActivity, R.layout.listview_app_item_title, null);
				...
				if(convertView == null){
					convertView = View.inflate(mActivity, R.layout.listview_process_item, null);
				...
				String strSize = Formatter.formatFileSize(mActivity, getItem(position).memSize);
				holder.tv_memory_info.setText(strSize);
				
				//本进程不能被选中,所以先将checkbox隐藏掉
				if(getItem(position).packageName.equals(mActivity.getPackageName())){
	...
	private void getData() {
		new Thread(){
			public void run() {
				mProcessInfoList = ProcessInfoProvider.getProcessInfo(mActivity);
	...
	private void initTitleData() {
		mProcessCount = ProcessInfoProvider.getProcessCount(mActivity);
		tv_process_count.setText("进程总数:"+mProcessCount);
		
		//获取可用内存大小,并且格式化
		mAvailSpace = ProcessInfoProvider.getAvailSpace(mActivity);
		String strAvailSpace = Formatter.formatFileSize(mActivity, mAvailSpace);
		
		//总运行内存大小,并且格式化
		long totalSpace = ProcessInfoProvider.getTotalSpace(mActivity);
		mStrTotalSpace = Formatter.formatFileSize(mActivity, totalSpace);
		
		tv_memory_info.setText("剩余/总共:"+strAvailSpace+"/"+mStrTotalSpace);
	}
	
	private void initUI() {
		tv_process_count = (TextView) mview.findViewById(R.id.tv_process_count);
		tv_memory_info = (TextView) mview.findViewById(R.id.tv_memory_info);
		
		tv_des = (TextView) mview.findViewById(R.id.tv_des);
	
		lv_process_list = (ListView) mview.findViewById(R.id.lv_process_list);
			...
			public void onItemClick(AdapterView<?> parent, View view,int position, long id) {
				if(position == 0 || position == mCustomerList.size()+1){
					...
					if(mProcessInfo!=null){
						if(!mProcessInfo.packageName.equals(mActivity.getPackageName())){
	...
	private void clearAll() {
		...
		for(ProcessInfo processInfo:mCustomerList){
			if(processInfo.getPackageName().equals(mActivity.getPackageName())){
				continue;
			}
...
//				mCustomerList.remove(processInfo);
		...
		for (ProcessInfo processInfo : killProcessList) {
			...
			ProcessInfoProvider.killProcess(mActivity,processInfo);
		...
		//11,根据进程总数和剩余空间大小
		tv_process_count.setText("进程总数:"+mProcessCount);
		tv_memory_info.setText("剩余/总共"+Formatter.formatFileSize(mActivity, mAvailSpace)+"/"+mStrTotalSpace);
		//12,通过吐司告知用户,释放了多少空间,杀死了几个进程,
		String totalRelease = Formatter.formatFileSize(mActivity, totalReleaseSpace);
